...
public class MergeKSortedLinkedList {
	...
	private List<Integer> mergeksortedlikedlist(List<List<Integer>> lists, int k) {
		...
		while (!queue.isEmpty()) {
			...
			if (node.listIndex == lists.size() - 1 && node.index == lists.get(node.listIndex).size() - 1) {
				break;
			}
	...
	public static void main(String[] args) {
		...
		List<Integer> result= mergeKSortedLinkedList.mergeksortedlikedlist(input, 2);
