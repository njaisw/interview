...
/**
 * Swap two nodes in double link list. If it swaps first node its callers
 * responsibility to fix the head Test cases A right neighbor of B B right
 * neighbor of A A and B not neighbors of each other A or B are start or end
 * nodes
 */
public class SwapTwoNodesInDoubleLL {
	...
	public void swap(Node nodeA, Node nodeB) {
		...
		//TODO  if B is right neighbor of A
		if (nodeA.next == nodeB) {
			...
			nodeB.next = nodeA;
			nodeA.before = nodeB;
		} // TODO else if A is right neighbor of B
			...
			nodeA.next = nodeB;
			nodeB.before = nodeA;
		} //TODO if A and B are not neighbors of each other
		else {
			//NodeA is not first node
			if (nodeA.before != null) {
				nodeA.before.next = nodeB;
			}
			//NodeA is not last node
			if (nodeA.next != null) {
				nodeA.next.before = nodeB;
			}
			...
			nodeB.before = nodeA.before;
			nodeB.next = nodeA.next;
            //If node B is last node
			if (next != null) {
				next.before = nodeA;
			}
			//if nodeb is not first node
			if (before != null) {
				before.next = nodeA;
			}
			nodeA.before = before;
