 ...
 */
public class _04a_LongestPalindromeSubstring_1 {
    ...
    public int longestPalindromeDynamic(char []str){
        ...
        for(int l = 2; l <= str.length; l++){
            int len = 0;
            //TODO row goes till total length - len +1
            for(int row=0; row < str.length-l+1; row++){
                int col = row + l-1;
                len = 0;
                if(l == 2){
                    if(str[row] == str[col]){
                    ...
                    if(str[row] == str[col] && T[row+1][col-1]){
                        T[row][col] = true;
                        //TODO len is computed every step here which is column - row +1
                        len = col -row + 1;
                    }
                }
                if(len > max){
    ...
    public static void main(String args[]) {
        _04a_LongestPalindromeSubstring_1 lps = new _04a_LongestPalindromeSubstring_1();
