 ...
 * @Threadsafe
 */
public class X_RealTimeCounter {

    private final static int GRANULARITY = 300;
    private AtomicLongArray counter = new AtomicLongArray(GRANULARITY);
    private volatile int pos = 0;
    
    private X_RealTimeCounter(){
        PositionUpdater positionUpdater = new PositionUpdater(this);
        positionUpdater.start();
    }
    //Singleton
    private static volatile X_RealTimeCounter INSTANCE;
    
    public static X_RealTimeCounter getInstance(){
        if(INSTANCE == null){
        	//TODO Lock on Class Object
            synchronized (X_RealTimeCounter.class) {
                if(INSTANCE == null){
                    INSTANCE = new X_RealTimeCounter();
                }
            }
        }
        return INSTANCE;
    ...
    public static void main(String args[]){
        ...
        X_RealTimeCounter realTimeCounter = new X_RealTimeCounter();
...
class PositionUpdater extends TimerTask{
    ...
    private final X_RealTimeCounter realTimeCounter;
    private final Timer timer = new Timer(true);
    private static final int DELAY = 1000;
    PositionUpdater(X_RealTimeCounter realTimeCounter) {
