...
/**
 * Given a list of lists. Each element in the list is sorted. Sort the entire
 * list. Test cases One or more lists are empty All elements in one list are
 * smaller than all elements in another list
 */
public class ChunkMerge_IMP_3 {

	//TODO implements Comparable here
	...
	private void sortedMerge(List<Integer> result, int start, int end, int sum[]) {
		...
		/**
		 * If chunks are of equal size then i = size*start to (mid+1)*size -1 j =
		 * (mid+1)*size to size*(end+1)
		 */
	...
	public static void main(String args[]) {
		...
		ChunkMerge_IMP_3 cm = new ChunkMerge_IMP_3();
