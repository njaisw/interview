...
public class _3d_KruskalMST {
    ...
    public List<Edge<Integer>> getMST(_1_Graph<Integer> graph) {
        ...
        for (Edge<Integer> edge : allEdges) {
            //TODO get the sets of two vertices of the edge
            long root1 = disjointSet.findSet(edge.getVertex1().getId());
            long root2 = disjointSet.findSet(edge.getVertex2().getId());

            //TODO check if the vertices are in same set or different set
            //TODO if verties are in same set then ignore the edge
            if (root1 == root2) {
                continue;
            } else {
                // TODO if vertices are in different set then add the edge to result and union these two sets into one
                resultEdge.add(edge);
    ...
    public static void main(String args[]) {
        ...
        graph.addEdge(3, 4, 5);
        graph.addEdge(3, 7, 8);
        _3d_KruskalMST mst = new _3d_KruskalMST();
