 ...
 * CLRS book*/
public class _3_DijkstraShortestPath {

    //TODO returns distance map
    public Map<Vertex<Integer>,Integer> shortestPath(_1_Graph<Integer> graph, Vertex<Integer> sourceVertex){
        ...
        while(!minHeap.empty()){
            ...
            _7_BinaryMinHeap<Vertex<Integer>>.Node heapNode = minHeap.extractMinNode();
            Vertex<Integer> current = heapNode.key;

            //TODO distance is put only for the latest vertex. update shortest distance of current vertex from source vertex
            distance.put(current, heapNode.weight);

            //TODO iterate through all edges of current vertex
            for(Edge<Integer> edge : current.getEdges()){

                //get the adjacent vertex
                Vertex<Integer> adjacent = getVertexForEdge(current, edge);

                //TODO if heap does not contain adjacent vertex means adjacent vertex already has shortest distance from source vertex
    ...
    public static void main(String args[]){
        ...
        _3_DijkstraShortestPath dsp = new _3_DijkstraShortestPath();
