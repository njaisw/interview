 ...
 * CLRS book
 */
public class _3a_PrimMST {

    /**
     * Main method of Prim's algorithm.
     *
     * 1: It has vertex-edge map
     * 2: it has new distance does not inlcude the previous index burden
     */
    //Returns list of edges
    public List<Edge<Integer>> primMST(_1_Graph<Integer> graph){

        //binary heap + map data structure
        _7_BinaryMinHeap<Vertex<Integer>> minHeap = new _7_BinaryMinHeap<>();

        //TODO map of vertex to edge which gave minimum weight to this vertex.
        Map<Vertex<Integer>,Edge<Integer>> vertexEdgeHashMap = new HashMap<>();

        //Todo  stores final result
        ...
        while(!minHeap.empty()){
            ...
            Edge<Integer> spanningTreeEdge = vertexEdgeHashMap.get(current);
            // TODO only polled edges goes to the final result of edges
            if(spanningTreeEdge != null) {
                result.add(spanningTreeEdge);
            }

            //TODO iterate through all the adjacent edges
            for(Edge<Integer> edge : current.getEdges()){
                Vertex<Integer> adjacent = getVertexForEdge(current, edge);
                //TODO check if adjacent vertex exist in heap + map and weight attached with this vertex is greater than this edge weight
                if(minHeap.containsData(adjacent) && minHeap.getWeight(adjacent) > edge.getWeight()){
                    ...
                    vertexEdgeHashMap.put(adjacent, edge);
                }
            }
        }
        return result;
    ...
    public static void main(String args[]){
     ...
     /* graph.addEdge(0, 1, 4);
        ...
        _3a_PrimMST prims = new _3a_PrimMST();
