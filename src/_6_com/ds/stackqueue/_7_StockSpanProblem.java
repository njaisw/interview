 ...
 * Question: The stock span
 * problem is a financial problem where we have a series of n daily price quotes
 ...
 * 1, 4, 6}
 *
 * Solution 1: We see that S[i] on day i can be easily computed if we know the
 * closest day preceding i, such that the price is greater than on that day than
 * the price on day i. If such a day exists, let’s call it h(i), otherwise, we
 * define h(i) = -1. The span is now computed as S[i] = i – h(i). See the
 * following diagram.
 */
public class _7_StockSpanProblem {
	public static int[] stockSpan(int[] prices) {
		//TODO Deque - LinkedList, ArrayDeck
		...
		for (int i = 1; i < prices.length; i++) {
			...
			//TODO if stack is empty the i+1
			if (stack.isEmpty()) {
				stockSpan[i] = i + 1;
			} else {
